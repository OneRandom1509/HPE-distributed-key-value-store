...
int main(int argc, char **argv)
{
  ...
  if(argc >= 5)
    {
      ...
      if(mode_arg == "persistent")
        ...
        {
          std::cout << "[Server] Unknown storage mode '" << mode_arg
    ...
    {
      std::cout
        << "[Server] No storage mode specified, defaulting to MEMORY\n";
    }
  ...
  std::cout << "Initializing KvStore with " << (mem_size / (1024 * 1024))
            << "MB in "
            << (storage_mode == StorageMode::PERSISTENT ? "PERSISTENT"
                                                        : "MEMORY")
            << " mode\n";
  ...
  std::cout << "KVServer started with provider ID: " << provider_id
            << std::endl;
  std::cout << "Server is running. Connect using: " << myEngine.self()
            << std::endl;
