                               ...
                               ConnectionMode conn_mode)
{
  std::cout << "[get_instance] Initializing KvStore with " << (size / MB)
            << "MB in "
            << (mode == StorageMode::PERSISTENT ? "PERSISTENT" : "MEMORY")
            << " mode as "
  ...
  try
    {
      ...
      PrintMemoryStats(conn_mode == ConnectionMode::SERVER
                         ? "Server Initialization"
                         : "Client Connection");
    }
  catch(const interprocess_exception &e)
    {
      std::cout << "[KvStore] Caught interprocess exception: " << e.what()
                << std::endl;

      if(conn_mode == ConnectionMode::CLIENT)
        {
          std::cout
            << "[KvStore] Client failed to connect to existing storage. "
      ...
      try
        ...
        {
          std::cout << "[KvStore] Failed to open existing storage: "
                    << e2.what() << std::endl;
      ...
      if(result.first != nullptr)
        {
          std::cout << "[KvStore] Found existing shared memory with "
                    << result.first->size()
                    << " entries, will remove and recreate\n";
        }
    }
  catch(const interprocess_exception &e)
    {
      std::cout << "[KvStore] No existing shared memory found (expected): "
                << e.what() << std::endl;
    }
  ...
  try
    {
      ...
      memory_map_ptr = memory_storage->construct<MemoryHashMap>("SharedMap")(
        0, boost::hash<int>(), std::equal_to<int>(), allocator);

      std::cout << "[KvStore] Created new memory-based unordered map\n";
    }
  catch(const interprocess_exception &e)
    {
      std::cout << "[KvStore] Failed to create shared memory: " << e.what()
                << std::endl;
      throw;
    }
}
  ...
  if(file_exists)
    {
      try
          ...
          if(result.first != nullptr)
            {
              ...
              std::cout << "[KvStore] Successfully opened existing persistent "
                           "storage with "
            ...
            {
              std::cout << "[KvStore] File exists but no valid map found, "
                           "will recreate\n";
              file_storage.reset();
            }
        }
        ...
        {
          std::cout << "[KvStore] Failed to open existing file: " << e.what()
                    << std::endl;
          file_storage.reset();
        }
    }
  ...
  try
    {
      ...
      persistent_map_ptr = file_storage->construct<MappedHashMap>("SharedMap")(
        0, boost::hash<int>(), std::equal_to<int>(), map_allocator);
    ...
    {
      std::cout << "[KvStore] Failed to create persistent storage: "
                << e.what() << std::endl;
      throw;
    }
}
      ...
      if(result.first != nullptr)
        {
          ...
          std::cout << "[KvStore] Successfully connected to existing shared "
                       "memory with "
      ...
      else
        {
          throw interprocess_exception(
            "SharedMap not found in existing shared memory");
        }
    }
  catch(const interprocess_exception &e)
    {
      std::cout << "[KvStore] Failed to connect to existing shared memory: "
                << e.what() << std::endl;
      throw;
    }
}
      ...
      if(result.first != nullptr)
        {
          persistent_map_ptr = result.first;
          std::cout
            << "[KvStore] Successfully connected to existing persistent "
               "storage with "
            << persistent_map_ptr->size() << " entries\n";
        }
      else
        {
          throw interprocess_exception(
            "SharedMap not found in existing persistent storage");
        }
    }
  catch(const interprocess_exception &e)
    {
      std::cout
        << "[KvStore] Failed to connect to existing persistent storage: "
        << e.what() << std::endl;
      throw;
    }
}
      ...
      try
        {
          std::vector<std::string> files_to_remove
            = {"mappings.txt", "local_store_node_0.dat"};
              ...
              if(entry.is_regular_file())
                {
                  ...
                  if(filename.find("local_store_node_") == 0
  ...
  if(storage_mode == StorageMode::MEMORY && memory_storage)
    {
      ...
      stats.usage_percent
        = (static_cast<double>(stats.used_memory) / stats.total_size) * 100.0;
    }
  ...
  try
    {
      ...
      std::size_t entry_size
        = value.size() + sizeof(int) + 64; // Added overhead estimate
      ...
      if(storage_mode == StorageMode::MEMORY)
        {
          ...
          memory_map_ptr->insert(
            std::make_pair(key, MyShmString(value.c_str(), char_allocator)));
        }
      else
        {
          // Check if key already exists
          ...
          MappedCharAllocator char_allocator(
            file_storage->get_segment_manager());
          persistent_map_ptr->insert(std::make_pair(
            key, MappedShmString(value.c_str(), char_allocator)));
    ...
    {
      std::cout << "Error during insertion (bad_alloc): " << e.what()
                << std::endl;
          ...
          if(it != memory_map_ptr->end())
            {
              ...
              std::cout << "Key " << key << " updated to: " << new_value
                        << std::endl;
              PrintMemoryStats("After Update");
            }
          else
            {
              std::cout << "Key " << key << " not found. Cannot update."
                        << std::endl;
          ...
          if(it != persistent_map_ptr->end())
            {
              ...
              MappedCharAllocator char_alloc(
                file_storage->get_segment_manager());
      ...
      if(storage_mode == StorageMode::MEMORY)
        {
          ...
          if(it != memory_map_ptr->end())
            ...
            {
              std::cout << "Key " << key << " not found. Nothing to delete."
                        << std::endl;
          ...
          if(it != memory_map_ptr->end())
            {
              ...
              std::cout << "Found key " << key << " with value: " << normal_str
                        << std::endl;
            ...
            {
              std::cout << "Key " << key << " not found in storage."
                        << std::endl;
...
KvStore::~KvStore()
{
  ...
  if(conn_mode == ConnectionMode::CLIENT)
    {
      std::cout
        << "[KvStore] Client disconnecting, leaving shared resources intact"
        << std::endl;
      return;
    }

  std::cout << "[KvStore] Destructor called, cleaning up resources"
            << std::endl;
  ...
  try
    {
          ...
          if(memory_map_ptr)
            {
              ...
              std::cout << "Destroyed unordered map from shared memory."
                        << std::endl;
          ...
          if(persistent_map_ptr)
            {
              ...
              std::cout << "Destroyed persistent map from mapped file."
                        << std::endl;
      ...
      try
        ...
        {
          std::cout << "Warning: Could not clean up mutex: " << e.what()
                    << std::endl;
        }
    }
  catch(const std::exception &e)
