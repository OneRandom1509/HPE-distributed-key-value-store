...
KVDistributor::KVDistributor(KvStore &kv_store, const Config &config)
    : kv(kv_store), config(config), // bind config reference
      count_of_node(config.read_count()), protocol(config.read_protocol()),
      provider_id(1), kv_client(protocol, 1)
{
...
int KVDistributor::getNodeId(int key) { return key % count_of_node; }
  ...
  if(node_id == local_node_id)
  ...
  else
    {
      try
        ...
        {
          std::cerr << "Error fetching key " << key << ": " << e2.what()
                    << std::endl;
          return "RPC Failed";
        }
    }
  ...
  if(node_id == local_node_id)
    {
      try
        ...
        {
          std::cerr << "Error inserting key " << key << ": " << e.what()
                    << std::endl;
        }
    }
  else
    {
      try
        ...
        {
          std::cerr << "Error inserting key " << key
                    << " via RPC: " << e.what() << std::endl;
        }
    }
}
  ...
  if(node_id == local_node_id)
    {
      try
        ...
        {
          std::cerr << "Error updating key " << key << ": " << e.what()
                    << std::endl;
        }
    }
  else
    {
      try
        ...
        {
          std::cerr << "Error updating key " << key << " via RPC: " << e.what()
                    << std::endl;
        }
    }
}
  ...
  if(node_id == local_node_id)
    {
      try
        ...
        {
          std::cerr << "Error deleting key " << key << ": " << e.what()
                    << std::endl;
        }
    }
  else
    {
      try
        ...
        {
          std::cerr << "Error deleting key " << key << " via RPC: " << e.what()
                    << std::endl;
        }
    }
}
